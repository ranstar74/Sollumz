...
class ClipAnimation(bpy.types.PropertyGroup):
    ...
    def get_armature(self):
        if self.animation is None:
            return None
        return self.animation.animation_properties.armature
...
class ClipProperties(bpy.types.PropertyGroup):
    ...
    def validate_links(self):
        """Checks if all animation links share the same armature."""
        armatures = []
        for anim in self.animations:
            armature = anim.get_armature()
